...
struct Args
{
  ...
  std::string grammar = "";
  std::string thr_lce = "";
  size_t mem_size = 1; // the number of LCP computations to delay
...
void parseArgs(int argc, char *const argv[], Args &arg)
{
  ...
  std::string usage("usage: " + std::string(argv[0]) + " infile [-s store] [-m memo] [-c csv] [-p patterns] [-f fasta] [-d mem_size] [-r rle] [-g grammar] [-t thr_lce] [-b bytes]\n\n" +
                    ...
                    "mem_size:  [integer] - minimum mem size (def. 1)\n" +
